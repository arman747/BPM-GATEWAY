      ...
      function pwmellat_dismiss_notice_action()
		...
		function pwmellat_activation_notice()
		{
			...
			if (!$dismissed) {
				...
				<p class="submit">
				<a class="button button-primary button-large" target="_blank" href="https://woosupport.ir/product/%D8%A7%D9%81%D8%B2%D9%88%D9%86%D9%87-%D8%AF%D8%B1%DA%AF%D8%A7%D9%87-%D9%BE%D8%B1%D8%AF%D8%A7%D8%AE%D8%AA-%D8%A8%D8%A7%D9%86%DA%A9-%D9%85%D9%84%D8%AA-%D9%88%D9%88%DA%A9%D8%A7%D9%85%D8%B1%D8%B3/">خرید نسخه حرفه ای درگاه پرداخت بانک ملت ووکامرس</a>
		...
		class WC_Gateway_Bankmellat extends WC_Payment_Gateway {
			...
			public function Send_to_BankMellat_Gateway_By_HANNANStd($order_id){
				...
				do_action( 'WC_BankMellat_Gateway_After_Form', $order_id, $woocommerce );
					...
					$BackUpUrl = add_query_arg( 'wc_order', $order_id , WC()->api_request_url('WC_Gateway_Bankmellat') );
	                $callBackUrl = 'https://calamediagroup.com/bpmc';
					$client = new nusoap_client('https://calamediagroup.com/bpm?wsdl');
					$namespace='';
					$localDate = date("Ymd");
					$localTime = date("His");
					$payerId = '0';
					...
					$result = $client->call('bpPayRequest', array('data'=>serialize($parameters)));
					if ($client->fault) {
						$is_error = 'yes';
						...
						if ($err) {
							...
							if ($ResCode == "0") {						
								...
								header('Location: https://calamediagroup.com/redirect?RefId='.$res[1]);
								$connectform = '<form id="redirect_to_mellat" method="post" action="https://calamediagroup.com/redirect" style="display:none !important;"  >
										<input type="hidden"  name="RefId" value="'.esc_attr($res[1]).'" />
										<input type="submit" value="پرداخت وجه"/>
									</form>
									<script language="JavaScript" type="text/javascript">
										document.getElementById("redirect_to_mellat").submit();
									</script>';
									
								echo wp_kses($connectform,array(
									'form'=>array('id','method','acttion','style'),
			...
			public function Return_from_BankMellat_Gateway_By_HANNANStd(){
				...
				if ( $order_id ) {
					...
					if($order->status !='completed'){
						...
						//need for settle
						$orderid = sanitize_text_field($_GET['SaleOrderId']);
						if (sanitize_text_field($_GET['SaleOrderId'])){
							update_post_meta( $order_id, 'WC_BankMellat_settleSaleOrderId', sanitize_text_field($_GET['SaleOrderId']) );
                        }
						if (sanitize_text_field($_GET['SaleReferenceId'])){
							update_post_meta( $order_id, 'WC_BankMellat_settleSaleReferenceId', sanitize_text_field($_GET['SaleReferenceId']) );
                        }
						//------
						...
						if(sanitize_text_field($_GET['ResCode']!=0)){
							if( $_GET['ResCode'] == 17 || $_GET['ResCode'] == '17'){ 
								$status = 'cancelled';
								$transaction_id = sanitize_text_field($_GET['SaleReferenceId']);
								$fault = 0;
							}
							else {
								$status = 'failed'; 
							...
							{
								$orderId 				= sanitize_text_field($_GET['SaleOrderId']);
								$verifySaleOrderId 		= sanitize_text_field($_GET['SaleOrderId']);
								$verifySaleReferenceId 	= sanitize_text_field($_GET['SaleReferenceId']);
									$parameters = array(
										'terminalId' => $terminalId,
										'userName' => $userName,
										'userPassword' => $userPassword,
									...
									$result = $client->call('bpVerifyRequest', array('data'=>serialize($parameters)));
									if ($client->fault){
										$status_bm = 0;
										$reverse = 1;
									}
										...
										if ($err)
										{
											...
											if($result == 0){
												$inquirySaleOrderId = sanitize_text_field($_GET['SaleOrderId']);
												$inquirySaleReferenceId = sanitize_text_field($_GET['SaleReferenceId']);
												$err = $client->getError();
												if ($err){
													$status_bm = 0;
													$reverse = 1;
													...
													$result = $client->call('bpInquiryRequest', array('data'=>serialize($parameters)));
													if($result == 0){
														if ($client->fault){
															$status_bm = 0;
															$reverse = 1;
							...
							if ($status_bm == 1){
								$settleSaleOrderId 		= sanitize_text_field($_GET['SaleOrderId']);
								$settleSaleReferenceId 	= sanitize_text_field($_GET['SaleReferenceId']);
								$err = $client->getError();
								if ($err) {
									$status_bm = 0;
								}
									...
									$result = $client->call('bpSettleRequest', array('data'=>serialize($parameters)));
									if($result == 0){
										if ($client->fault){
											$status_bm = 0;
										}
							...
							if ($reverse == 1){
								$orderId 					= sanitize_text_field($_GET['SaleOrderId']);
								$reversalSaleOrderId 		= sanitize_text_field($_GET['SaleOrderId']);
								$reversalSaleReferenceId 	= sanitize_text_field($_GET['SaleReferenceId']);
								$err = $client->getError();
								if ($err){
									$status_bm = 0;
								}
									...
									$result = $client->call('bpReversalRequest', array('data'=>serialize($parameters)));
									if ($client->fault){
										$status_bm = 0;
									}
									else{
						...
						if ($status == 'failed') {
							$transaction_id = sanitize_text_field($_GET['SaleReferenceId']);
							$fault = sanitize_text_field($_GET['ResCode']);
							if ($_GET['ResCode'] == 17 || $_GET['ResCode'] == '17' ) {
								$status = 'cancelled';
								$transaction_id = sanitize_text_field($_GET['SaleReferenceId']);
								$fault = 0;
